...
Запуск:
    ...
    # project-key можно не передавать флагом, если он уже указан в input JSON:
    # python3 create_project.py --input client.input.json --jira-url ... \
    #     --email ... --api-token *** [--execute --confirm] [--create-sprints]

    # --lang ua -- украинский вариант контента (Milestones/WBS/Task); можно
    # также передать через поле "lang" в input JSON -- --lang имеет приоритет:
    # python3 create_project.py --input client.input.json --lang ua \
    #     --jira-url ... --email ... --api-token *** [--execute --confirm]
...
def run_pipeline(
    ...
    *,
    ...
    lang: явный параметр (CLI --lang) имеет приоритет над input_data["lang"]
    (тот же паттерн приоритета, что jira.project_key в v3.4); по умолчанию
    "ru", если не задано ни там, ни там. run_form.py не передаёт lang вовсе --
    значение всегда берётся из input_json тела запроса."""
    resolved_lang = lang or input_data.get("lang") or "ru"
    plan = assemble_plan(input_data, lang=resolved_lang)
...
def main() -> None:
    ...
    parser.add_argument(
        "--lang",
        choices=["ru", "ua"],
        help="Язык контента шаблона (Milestones/WBS/Task) -- ru или ua; приоритет: "
        "--lang > поле lang в input JSON > ru по умолчанию",
    )
    ...
    project_key = args.project_key or (input_data.get("jira") or {}).get("project_key")
    if not project_key:
        parser.error("--project-key обязателен (или укажите jira.project_key в input JSON)")
