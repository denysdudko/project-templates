...
## v3.8 -- Синхронизирован tasks/M1_tasks.ua.yaml с interview_checklist v3.7 (RU)
- `tasks/M1_tasks.ua.yaml`: перевод `interview_checklist` T-1.1.1, T-1.2.1,
  T-1.3.1 обновлён под актуальные RU-формулировки v3.7 (см. запись ниже) --
  UA-версия не была синхронизирована в момент правки v3.7, т.к. запрос тогда
  был ограничен RU-файлом (`open-issues.md`, было открыто как п.7).
- Только перевод текста; Task ID, `depends_on`/`used_by`, привязка к WBS не
  менялись. WBS-1.4 (T-1.4.1) не трогался -- его `interview_checklist` не
  менялся в v3.7 и уже был переведён 1:1 с RU.
- **Проверка**: количество пунктов `interview_checklist` в RU и UA совпадает
  1:1 для T-1.1.1 (5), T-1.2.1 (14), T-1.3.1 (7), T-1.4.1 (8, контроль).
- **Регрессия**: `assemble_plan.py --selftest` (включая блок `--lang ua` --
  совпадение Task ID, идентичные effort_estimates, различие текста
  `Task.name`) и `validate_plan.py --selftest` -- зелёные.
  `assemble_plan.py --input examples/client-abc.input.json --lang ua`
  проверен вручную на M1: UA-текст трёх Task отражает обновлённые вопросы,
  структура (ID/порядок Task) не нарушена.
- `open-issues.md`, п.7 закрыт.

## v3.7 -- Исправлено смешение уровней процесс/функция и дубли в interview_checklist M1 (T-1.1.1/T-1.2.1), добавлен вопрос о разграничении e-Sklep/e-Sale в T-1.3.1
- `tasks/M1_tasks.yaml`, `T-1.1.1` (интервью по бизнес-процессам): чек-лист
  переписан -- прежние пункты либо дублировали `T-1.2.1` (функциональные
  требования: повторное оформление заказов, история заказов, документы ERP),
  либо смешивали уровень процесса с уровнем функции. Новые 5 пунктов -- только
  процессный уровень (кто покупатель, как получает доступ к магазину, кто
  подтверждает регистрацию, какой ассортимент доступен, что происходит с
  заказом после оформления).
- `T-1.2.1` (интервью по функциональным требованиям): добавлены 4 пункта,
  унесённые из `T-1.1.1` -- доступ к истории заказов, повторное оформление
  ранее выполненных заказов, отображение документов ERP (счета, WZ) в личном
  кабинете, скачивание счетов покупателем. Это функциональный уровень, ему тут
  и место.
- `T-1.3.1` (интервью по интеграциям): добавлен вопрос о том, входит ли
  интеграция с маркетплейсами (Comarch e-Sale, Allegro, Ceneo) в объём
  проекта -- явно разграничивает e-Sklep (сам магазин) и e-Sale/маркетплейсы
  как потенциально отдельный объём работ, до того как заказчик перечисляет
  конкретные площадки.
- Только `interview_checklist` трёх Task; ID, `depends_on`, `used_by`,
  `source`, структура WBS/Milestones не менялись.
- **Регрессия**: `assemble_plan.py --selftest` и `validate_plan.py --selftest`
  -- зелёные. `agent/examples/client-abc.plan.json` пересобран (только текст
  `interview_checklist` T-1.1.1/T-1.2.1/T-1.3.1 изменился, остальное
  байт-в-байт идентично); `client-abc.validation-report.txt` -- 0 находок;
  `client-abc.jira-export.dryrun.log.txt` пересобран и не изменился
  (dry-run-лог показывает только summary/estimate/labels/sprint по Subtask,
  не текст interview_checklist).
- **UA-версия (`tasks/M1_tasks.ua.yaml`) не менялась** -- запрос был
  ограничен RU-файлом; `--lang ua` не проверяет содержательное соответствие
  `interview_checklist` между RU/UA (только совпадение ID и различие текста
  `Task.name`), поэтому расхождение не ловится `--selftest`. Зафиксировано в
  `open-issues.md`, п.7.
...
- Добавлен `docs/form-usage.md` — пошаговая инструкция для консультанта на Windows:
  ...
  `.env.example` (`copy`), запуск `python run_form.py` (не `python3` — на Windows такой
  команды нет), работа с формой (dry-run -> просмотр отчёта -> «Выполнить»), таблица
  диагностики («python не является внутренней или внешней командой», «не заданы
  переменные окружения», «локальный сервер недоступен», антивирус/файрвол спрашивает про
  сетевой доступ python.exe).
- Найдено при разборе реальной ошибки консультанта: форма открывалась не через
  `run_form.py` (например, напрямую как `file://`), из-за чего относительный
  `fetch('/plan', …)` не может обратиться к серверу — ограничение браузерной песочницы,
  не баг; автозапуск сервера кнопкой невозможен без нативного компонента (зафиксировано
  как v2-предложение, не реализуется этой правкой).
- Код (`agent/run_form.py`, `agent/client-input-form.html`) не менялся — правка только
  документационная. Сам код уже кросс-платформенный (`pathlib.Path`, `http.server`,
  `webbrowser` — без unix-специфичных вызовов), проверить дополнительно не требуется.
- `schema/milestones_wbs.yaml` и `tasks/M*_tasks.yaml` не затронуты.
